...
public final class SigfoxRestHandler {
    ...
    SigfoxRestHandler(File eventFile) {
        this.eventWriter = new EventWriter(eventFile);
    }

    public void advanced(DataAdvanced advanced) {
        Instant time = Instant.ofEpochSecond(advanced.time).truncatedTo(ChronoUnit.SECONDS);
        LocationEvent event = new LocationEvent(time.toString(), advanced.device, advanced.seqNumber,
                advanced.linkQuality, advanced.data);
